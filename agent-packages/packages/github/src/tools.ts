...
export function createGitHubToolsExport(config: GitHubConfig): ToolConfig {
  ...
  const tools: DynamicStructuredTool<any>[] = [
    new DynamicStructuredTool({
      ...
      description: 'Search issues or PRs within a specific repository based on status, keywords, and reporter',
      schema: z.object({
        repo: config.repo
          ? z.string().describe('Name of the repository to search in. This identifies which project to look for issues/PRs').optional().default(config.repo)
          : z.string().describe('Name of the repository to search in. This identifies which project to look for issues/PRs (required)'),
        owner: config.owner
          ? z.string().describe('Username or organization that owns the repository. This is the first part of the repository URL').optional().default(config.owner)
          : z.string().describe('Username or organization that owns the repository. This is the first part of the repository URL (required)'),
    ...
    new DynamicStructuredTool({
      ...
      description: 'Search for code within a specific repository using natural language or keywords',
      schema: z.object({
        repo: config?.repo
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        ...
        repo: config.repo
          ? z.string().describe('Name of the repository where the file will be created/updated').optional().default(config.repo)
          : z.string().describe('Name of the repository where the file will be created/updated'),
        path: z.string().describe('Full path to the file in the repository, including filename and extension (e.g., "docs/README.md")'),
        content: z.string().describe('The actual content to write to the file. For text files, this is the text content; for binary files, base64 encoded content'),
        message: z.string().describe('Git commit message describing what changes were made and why'),
        branch: z.string().describe('Name of the branch where the file should be created/updated (e.g., "main", "feature/new-docs")'),
        sha: z.string().describe('The SHA hash of the file being replaced. Required when updating an existing file, omit when creating new').optional()
      }),
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        name: z.string().describe('Name for the new repository. Should be URL-friendly, using letters, numbers, hyphens'),
        description: z.string().describe('A short description of what this repository is for. Helps others understand the project\'s purpose').optional(),
        private: z.boolean().describe('Whether the repository should be private (true) or public (false). Private repos are only visible to you and collaborators').optional(),
        autoInit: z.boolean().describe('Whether to initialize with a README file. Recommended true for new projects to provide initial documentation').optional()
      }),
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        owner: config.owner
          ...
          : z.string().describe('Username or organization that owns the repository'),
        repo: config.repo
          ? z.string().describe('Name of the repository where the PR will be created').optional().default(config.repo)
        ...
        head: z.string().describe('Name of the branch containing your changes (e.g., "feature/new-feature")'),
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        owner: config.owner
          ...
          : z.string().describe('Username or organization that owns the repository'),
        repo: config.repo
          ? z.string().describe('Name of the repository to list issues from').optional().default(config.repo)
          : z.string().describe('Name of the repository to list issues from'),
        state: z.enum(['open', 'closed', 'all']).describe('Filter issues by their state: open, closed, or all').optional(),
        sort: z.enum(['created', 'updated', 'comments']).describe('How to sort issues: by creation date, last update, or number of comments').optional(),
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        owner: config.owner
          ...
          : z.string().describe('Username or organization that owns the repository'),
        repo: config.repo
          ? z.string().describe('Name of the repository containing the issue').optional().default(config.repo)
        ...
        state: z.enum(['open', 'closed']).describe('Change issue state to open or closed').optional(),
        assignees: z.array(z.string()).describe('New list of GitHub usernames to assign (replaces existing assignees)').optional(),
        labels: z.array(z.string()).describe('New list of labels to apply (replaces existing labels)').optional(),
        milestone: z.number().describe('New milestone number to associate with the issue').optional()
      }),
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        q: z.string().describe('Search query using GitHub user search syntax. Can include location:, language:, followers:, or other qualifiers'),
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        owner: config.owner
          ...
          : z.string().describe('Username or organization that owns the repository'),
        repo: config.repo
          ? z.string().describe('Name of the repository containing the PR').optional().default(config.repo)
        ...
        event: z.enum(['APPROVE', 'REQUEST_CHANGES', 'COMMENT']).describe('Type of review: APPROVE to accept changes, REQUEST_CHANGES to request modifications, COMMENT for neutral feedback'),
        commit_id: z.string().describe('Specific commit SHA to attach the review to. Usually the latest commit in the PR').optional(),
        ...
        })).describe('Line-specific comments to add to the review').optional()
      }),
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        ...
        commit_title: z.string().describe('Title for the merge commit. If not provided, GitHub will generate one').optional(),
        commit_message: z.string().describe('Additional details for the merge commit message').optional(),
        merge_method: z.enum(['merge', 'squash', 'rebase']).describe('How to merge: "merge" creates merge commit, "squash" combines all commits, "rebase" adds commits individually').optional()
      }),
      func: async (params: MergePullRequestParams) => service.mergePullRequest(params)
    }),
    new DynamicStructuredTool({
      name: 'search_code_global',
      description: 'Search for code across all public GitHub repositories using keywords, file paths, or language',
      schema: z.object({
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        q: z.string().describe('Search query using GitHub issues search syntax. Can include is:issue/pr, state:open/closed, label:name, etc.'),
    ...
    new DynamicStructuredTool({
      ...
      schema: z.object({
        ...
        expected_head_sha: z.string().describe('Expected SHA of the PR\'s HEAD ref. Used to ensure the branch hasn\'t changed').optional()
      }),
