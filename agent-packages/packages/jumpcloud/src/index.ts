...
export class JumpCloudService implements BaseService<JumpCloudConfig> {
  ...
  async listGroups(params: z.infer<typeof SCHEMAS.listGroupsSchema>): Promise<ListGroupsResponse> {
    try {
      ...
      return { success: false, error: error instanceof Error ? error.message : 'Failed to list groups' };
    }
  }
  ...
  async assignUserToGroup(args: z.infer<typeof SCHEMAS.assignUserToGroupSchema>): Promise<AssignUserToGroupResponse> {
    try {
      await this.client.post(`/v2/usergroups/${args.groupId}/members`, { id: args.userId });
      ...
      return { success: false, error: error instanceof Error ? error.message : 'Failed to assign user to group' };
    }
  }

  async unassignUserFromGroup(args: z.infer<typeof SCHEMAS.unassignUserFromGroupSchema>): Promise<UnassignUserFromGroupResponse> {
    try {
      await this.client.delete(`/v2/usergroups/${args.groupId}/members/${args.userId}`);
      ...
      return { success: false, error: error instanceof Error ? error.message : 'Failed to unassign user from group' };
    }
  }

  async listGroupUsers(args: z.infer<typeof SCHEMAS.listGroupUsersSchema>): Promise<ListGroupUsersResponse> {
    try {
      ...
      return { success: false, error: error instanceof Error ? error.message : 'Failed to list group users' };
    }
  }
