...
export class QuixCallBackManager extends BaseCallbackHandler {
  ...
  handleToolEnd(output: Record<string, any>, runId: string, parentRunId?: string, tags?: string[], metadata?: Record<string, unknown>, runName?: string): any {
    ...
    if (this.captureToolCalls && this.toolCalls[runId]) {
      if (!isUndefined(output.content) && typeof output.content === 'string') {
        ...
        this.toolCalls[runId].result = output.content.length > 1000 ? output.content.slice(0, 1000) + `...redacted` : output.content
          ;
      }
    }
  }
